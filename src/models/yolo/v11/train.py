...
def parse_args():
    parser = argparse.ArgumentParser(description="Train YOLOv11 using the shared Ultralytics trainer.")
    ...
    parser.add_argument("--config", default=DEFAULT_CONFIG, help="Path to the YOLOv11 training config.")
...
def train_all_public_datasets():
    ...
    for config_path in configs:
        for dataset_name, data_yaml_path in datasets:
            print(f"Training YOLOv11 config={config_path} dataset={dataset_name}")
